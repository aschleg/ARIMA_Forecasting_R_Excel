...
find.best.arima <- function(Data, maxord=c(2,1,1,2,1,1))
{
	if(!is.ts(Data))
		stop
	...
	if(length(Data) < 5 * frequency(Data))
		stop
	...
	for(p in 0:maxord[1]) for(d in 0:maxord[2]) for(q in 0:maxord[3])
	{
		...
		{
			fit <- arima(Data, order=c(p,q,d),seas=list(order=c(P,D,Q),
				frequency(Data)), method='CSS')
